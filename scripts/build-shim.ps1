...
# Референсы — все DLL из dependencies.
# Если есть _public-версия (после публичайзера) — берём её и исключаем обычную (иначе дубль типов).
$hasAsmPublic = Test-Path (Join-Path $deps "Assembly-CSharp_public.dll")
$hasMirrorPublic = Test-Path (Join-Path $deps "Mirror_public.dll")
$refs = Get-ChildItem $deps -Filter *.dll | Where-Object {
    $_.Name -ne "Qurre.dll" -and
    -not ($hasAsmPublic -and $_.Name -eq "Assembly-CSharp.dll") -and
    -not ($hasMirrorPublic -and $_.Name -eq "Mirror.dll")
