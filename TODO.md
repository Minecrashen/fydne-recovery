...
> 📌 **Временный хостинг на первое время:** французская нода **Hostkey `82.40.49.127`**
> (из проекта Зоркофф). Слабая и **уже занята** — там крутится REALITY-выход (Xray) для
> Telegram-трафика Зоркофф. ⚠️ НЕ ронять `xray-fr`/REALITY. Реально влезет только **лёгкий
> Node-бэкенд** (`scp-socket` + `loli-api`, ~1 ГБ) для теста связки игра↔БД — **игровой
> сервер SCP:SL туда НЕ ставить** (нужно 2–4 ГБ + быстрое ядро, забьёт ноду и помешает REALITY).
> Полноценный игровой инстанс — на отдельном железе (см. ниже).

- [ ] (старт) Поднять на фр-ноде `82.40.49.127` лёгкий бэкенд для теста — НЕ трогая Xray
...
- [ ] Закрыть роль C#-разработчика в команде
