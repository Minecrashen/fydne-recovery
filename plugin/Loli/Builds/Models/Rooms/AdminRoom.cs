...
static class AdminRoom
{
    ...
    {
        // Светлые поверхности + сильный свет: dedicated-сервер не даёт рассчитывать на шейдеры/GI,
        // поэтому комната должна быть видимой даже при минимальном освещении клиента.
        Color32 floor = new(150, 154, 162, 255);
        Color32 wall = new(176, 180, 188, 255);
        Color32 accent = new(96, 102, 112, 255);
        Color32 glass = new(120, 180, 220, 90);
        Color32 trim = new(255, 210, 80, 255);

        // Нижний (tutorial) этаж.
        ...
        // Яркая кромка верхней платформы — ориентир, где заканчивается пол.
        AddBox(model, trim, new(0, 4.18f, 7.55f), Vector3.zero, new(16, 0.06f, 0.18f), false);
        AddBox(model, trim, new(0, 4.18f, -4.35f), Vector3.zero, new(16, 0.06f, 0.18f), false);
        AddBox(model, trim, new(7.95f, 4.18f, 1.6f), Vector3.zero, new(0.18f, 0.06f, 12), false);
        AddBox(model, trim, new(-7.95f, 4.18f, 1.6f), Vector3.zero, new(0.18f, 0.06f, 12), false);

        // Стеклянный фронт + барьер, чтобы с waiting-платформы нельзя было выйти в пустоту.
        AddBox(model, glass, new(0, 5.7f, -4.2f), Vector3.zero, new(12, 2.6f, 0.12f), true);
        AddBox(model, accent, new(0, 4.45f, -4.2f), Vector3.zero, new(16, 0.18f, 0.16f), true);
        ...
        AddBox(model, accent, new(6, 5.7f, -4.2f), Vector3.zero, new(0.18f, 2.6f, 0.16f), true);
        AddBox(model, accent, new(-7, 5.7f, -4.2f), Vector3.zero, new(2, 2.6f, 0.12f), true);
        AddBox(model, accent, new(7, 5.7f, -4.2f), Vector3.zero, new(2, 2.6f, 0.12f), true);

        AddBox(model, new Color32(120, 120, 128, 255), new(-7, 1.3f, -5.8f), Vector3.zero, new(4, 0.42f, 2), true);
        AddBox(model, new Color32(120, 120, 128, 255), new(7, 1.3f, -5.8f), Vector3.zero, new(4, 0.42f, 2), true);

        // Свет: верхняя платформа, нижний этаж и угловые заполнители.
        model.AddPart(new ModelLight(model, new Color32(245, 248, 255, 255), new(0, 7.4f, 1.6f), lightIntensity: 4.5f, lightRange: 38));
        model.AddPart(new ModelLight(model, new Color32(235, 240, 255, 255), new(0, 3.6f, -3.5f), lightIntensity: 3.5f, lightRange: 28));
        model.AddPart(new ModelLight(model, new Color32(255, 244, 224, 255), new(6, 6.4f, 5.5f), lightIntensity: 2.5f, lightRange: 20));
        model.AddPart(new ModelLight(model, new Color32(255, 244, 224, 255), new(-6, 6.4f, 5.5f), lightIntensity: 2.5f, lightRange: 20));
        model.AddPart(new ModelLight(model, new Color32(255, 244, 224, 255), new(6, 2.8f, 5), lightIntensity: 2.5f, lightRange: 20));
        model.AddPart(new ModelLight(model, new Color32(255, 244, 224, 255), new(-6, 2.8f, 5), lightIntensity: 2.5f, lightRange: 20));

        Log.Custom($"AdminRoom recovery shell spawned at {model.GameObject.transform.position} (world), lights=6: fallback primitives are active even if AdminRoom.json is missing.", "FYDNE-BUILD", System.ConsoleColor.DarkCyan);
    }
